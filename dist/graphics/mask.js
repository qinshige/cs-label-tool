...
export default class CSLabelMask {
    ...
    drawMaskMousedow(e, template) {
        if (e.buttons === 1) {
            ...
            document.getElementById('svg_draw_layer').appendChild(this.canvas);
    ...
    drawMaskMouseup(e) {
        new Promise((resolve, reject) => {
            ...
            const x_min = Math.min.apply(null, this.lineToMoveData.map(item => item.x));
            const y_min = Math.min.apply(null, this.lineToMoveData.map(item => item.y));
            const x_max = Math.max.apply(null, this.lineToMoveData.map(item => item.x));
            const y_max = Math.max.apply(null, this.lineToMoveData.map(item => item.y));
            const results = {
    ...
    convertCanvasToImage(results) {
        ...
        ctx.drawImage(this.canvas, results.x, results.y, results.width, results.height, 0, 0, results.width, results.height);
        var image = new Image();
    ...
    createSvgDOM() {
        const { x, y, width, height, img } = this.drawResultsList[this.drawResultsList.length - 1];
        compiler.Render({
            tag: 'g',
            type: 'svg',
            props: {
